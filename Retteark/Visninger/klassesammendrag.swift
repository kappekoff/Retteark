    ...
    func finnKarakterSammensetning() async -> [Karakter] {
        ...
        for deltaker in deltakere {
            for j in 0..<karakterer.count {
                let karakter =  karakterView(prøveId: prøve?.id ?? "", deltaker: deltaker, oppgaver: oppgaver, poenger: $poenger, indeks: 0, låstKarakter: Binding.constant(deltaker.låstKarakter), endretPoeng: Binding.constant(0)).finnKarakter()
